...
if ( file_exists(__DIR__ . '/utils.php') ) {
	...
	$project_composer = dirname(dirname(__DIR__)) . '/composer.json';
...
class FeatureContext extends BehatContext implements ClosuredContextInterface {
	...
	public function replace_variables($str) {
		$ret = preg_replace_callback('/\{([A-Z_]+)\}/', [$this, '_replace_var'], $str);
	...
	public function install_wp($subdir = '') {
		...
		$url          = empty($subdir) ? 'http://example.com' : 'http://' . basename($subdir) . '.com';
		$install_args = [
			'url'            => $url,
			'title'          => 'WP CLI Site',
			'admin_user'     => 'admin',
		...
		if ( $install_cache_path && file_exists($install_cache_path) ) {
			...
			if ( $install_cache_path ) {
				...
				self::run_sql('mysqldump --no-defaults', ['result-file' => "{$install_cache_path}.sql"], true /*add_database*/);
			}
		}
	}
