...
class PostsCommand extends MUMigrationBase {
	...
	public function update_author($args = [], $assoc_args = [], $verbose = true) {
			...
			function ($result) use (&$equals_id, &$author_not_found, $ids_map, $verbose, $is_woocommerce) {
				...
				if ( isset($ids_map->{$author}) ) {
					if ( $author != $ids_map->{$author} ) {
						...
						$this->log(
							sprintf(
								__('Updated post_author for "%1$s" (ID #%2$d)', 'mu-migration'),
								$result->post_title,
								absint($result->ID)
							),
							$verbose
				...
				foreach ( array_filter($uid_fields) as $f ) {
					...
					if ( isset($ids_map->{$old_user}) && $old_user != $ids_map->{$old_user} ) {
						...
						$this->log(
							sprintf(
								__('Updated %1$s for "%2$s" (ID #%3$d)', 'mu-migration'),
								$f,
								$result->post_title,
								absint($result->ID)
							),
		...
		if ( ! empty($author_not_found) ) {
			$this->warning(
				sprintf(
					__('%1$d records failed to update its post_author: %2$s', 'mu-migration'),
