...
class UsersCommand extends MUMigrationBase {
	 ...
	 */
	public function update_passwords($args = [], $assoc_args = []) {
		$this->process_args(
			[
				0 => '', // New password.
			],
		...
		$users_args = [
			...
			'include' => ! empty($this->assoc_args['include']) ? explode(',', $this->assoc_args['include']) : [],
			'exclude' => ! empty($this->assoc_args['exclude']) ? explode(',', $this->assoc_args['exclude']) : [],
		];
		...
		foreach ( $users as $user ) {
			...
			WP_CLI::log(sprintf(__('Password updated for user #%1$d:%2$s', 'mu-migration'), $user->data->ID, $user->data->user_login));
	...
	private function send_reset_link($user_data) {
		 ...
		 * @return string              Default email title.
		 */
		 ...
		 * @param string   $user_login The username for the user.
		 * @param \WP_User $user_data  WP_User object.
		 * @return string              Default mail message.
		 */
